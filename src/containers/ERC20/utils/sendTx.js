function sendTx(instance, signature, args, accounts, callback) {
  ...
  const defaultCallback = {
    ...
    receipt: null,
    error: (x, y) =>
      console.log('The error is ', x, ' and receipt is: ', y),
  }
    ...
    instance.methods[signature](...args)
      ...
      .on('confirmation', function(confirmationNumber, receipt) {
        if (callback['confirmation']) {
          callback['confirmation'](confirmationNumber, receipt)
        }
        console.log(
          `Confirmation Number: ${confirmationNumber} and Receipt: ${receipt}`,
        )
      })
